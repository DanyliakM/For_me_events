...
void loop() {
  ...
  if (win() == true) {
    for (int i = 0; i < 8; i++) {
      for (int j = 0; j < 16; j++) {
        // Show a simple win pattern: turn on all LEDs briefly
        gb.setLed(i, j, true);
      }
    }
    delay(2000);
...
void makeMove() {
  ...
  if (gb.getKey() == 1) {
    // compute next rotation index safely (wrap around)
    int nextRot = (rot == 3) ? 0 : rot + 1;
    if (!gb.checkBlockCollision(gb.block[nextRot], x + 1, y)) {
      rot = nextRot;
    }
  }
  if (gb.getKey() == 6) {
